...
import { updateCitizenNotificationPreferences } from '@/lib/actions/citizens';
import { z } from 'zod';
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (result.success) {
      ...
      return NextResponse.json({ success: false, error: result.error || 'Failed to update preferences.' }, { status: 500 });
    }
  } catch (error) {
